...
def get_args():
    ...
    """
    parser.add_argument("--model_name", default="last_model5.pkl", type=str, help="Model name to save and use")
    ###################################################
    ###################################################
...
class RCCarPolicy(Node):
    ...
    def train(self):
        ...
        best_model_path = os.path.join(self.model_dir, "best_model5.pkl")
        ...
        np.save(os.path.join(self.model_dir, "obs_mean_l5.npy"), self.obs_mean)
        np.save(os.path.join(self.model_dir, "obs_std_l5.npy"), self.obs_std)
        np.save(os.path.join(self.model_dir, "act_mean_l5.npy"), self.act_mean)
        np.save(os.path.join(self.model_dir, "act_std_l5.npy"), self.act_std)
    ...
    def load(self):
        ...
        if self.mode == 'val':
            ...
            self.obs_mean = np.load(os.path.join(self.model_dir, "obs_mean_l5.npy"))
            self.obs_std = np.load(os.path.join(self.model_dir, "obs_std_l5.npy"))
            self.act_mean = np.load(os.path.join(self.model_dir, "act_mean_l5.npy"))
            self.act_std = np.load(os.path.join(self.model_dir, "act_std_l5.npy"))
