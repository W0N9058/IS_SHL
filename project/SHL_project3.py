...
def get_args():
    ...
    """
    parser.add_argument("--model_name", default="best_model_13.pkl", type=str, help="Model name to save and use")
    ###################################################
    ###################################################
...
class RCCarPolicy(Node):
    ...
    def train(self):
        ...
        np.save(os.path.join(self.model_dir, "obs_mean_b13.npy"), self.obs_mean)
        np.save(os.path.join(self.model_dir, "obs_std_b13.npy"), self.obs_std)
        np.save(os.path.join(self.model_dir, "act_mean_b13.npy"), self.act_mean)
        np.save(os.path.join(self.model_dir, "act_std_b13.npy"), self.act_std)
    ...
    def load(self):
        ...
        if self.mode == 'val':
            ...
            self.obs_mean = np.load(os.path.join(self.model_dir, "obs_mean_b13.npy"))
            self.obs_std = np.load(os.path.join(self.model_dir, "obs_std_b13.npy"))
            self.act_mean = np.load(os.path.join(self.model_dir, "act_mean_b13.npy"))
            self.act_std = np.load(os.path.join(self.model_dir, "act_std_b13.npy"))
